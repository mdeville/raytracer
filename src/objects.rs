use crate::utils::{quadratic_solve, Hit, Ray};
...
impl Object for Cylinder {
    fn intersect(&self, ray: Ray) -> Option<Hit> {
        ...
        let b = 2.0
            * (ray.origin.dot(&ray.direction) - self.position.dot(&ray.direction) + an * dn
                - on * dn);
        let c = self.position.dot(&self.position) - 2.0 * self.position.dot(&ray.origin)
