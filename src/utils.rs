...
pub fn raytrace(scene: &Scene, ray: Ray, depth: u32) -> Vector3<f64> {
    ...
    let (object, hit) = match scene.find_closest(ray) {
    ...
    if reflection < 1.0 {
        for light in scene.lights.iter() {
            ...
            if !in_shadow {
                ...
                out_color += (hit.normal.dot(&shadow_ray.direction) * light.brightness() * light.color()).component_mul(&object.color()) + reflection_ray.dot(&-ray.direction).max(0.0).powf(10.0) * light.color() * light.brightness();
            }
        }
    }
